# codex-blog v1.7.1: Demo Runbook
...
> credentials live in `~/.config/codex-seo/google-api.json` (also
...
populated by sourcing the gitignored `.env.local`. Restart Codex
after sourcing so the MCP subprocess inherits the values.

```bash
cd /home/agricidaniel/Desktop/codex-blog
source .env.local
...
# Then restart Codex so MCP servers pick up the env
```
...
| DataForSEO live SERP, keywords, backlinks, AI visibility | `seo-dataforseo` skill (in codex-seo) | MCP wired, restart needed |
...
| Symptom | Cause | Fix |
|---|---|---|
| `${GOOGLE_AI_API_KEY}` shows as literal in MCP env | Codex launched before `source .env.local` | `source .env.local` then restart Codex |
...
| "MCP not configured" from `/blog image` | MCP didn't load this session | Check `.mcp.json` exists, then restart Codex |
...
$EDITOR ~/.config/codex-seo/google-api.json    # mode 0600
...
shred -u ~/.config/codex-seo/google-api.json
...
`~/.config/codex-seo/{oauth-token,google-api}.json` automatically.
...
- `.env.local` (mode 0600, gitignored): the literal credentials live
  here. Source it before launching Codex.
- `~/.config/codex-seo/google-api.json` (mode 0600, user-private):
