...
Common issues, their causes, and fixes for `codex-blog`. Issues are grouped
...
**Cause**: Codex caches skill definitions at startup. New skills are not
detected until the CLI is restarted.

**Fix**:
1. Close Codex completely (exit the CLI or close the terminal)
2. Reopen Codex
...
irm https://raw.githubusercontent.com/AgriciDaniel/codex-blog/main/install.ps1 | iex
```
...
1. **Templates not installed**: Verify the templates directory exists:
   ```bash
   ls ${CODEX_HOME:-$HOME/.codex}/skills/blog/templates/
   ```
   If empty or missing, re-run `./install.sh`.

2. **Wrong install path**: Templates must be in
   `${CODEX_HOME:-$HOME/.codex}/skills/blog/templates/`, not in the repository's
   `skills/blog/templates/` directory.

3. **Template file corrupted**: Re-copy from the repository:
   ```bash
   cp skills/blog/templates/*.md ${CODEX_HOME:-$HOME/.codex}/skills/blog/templates/
   ```
...
1. **Agent file not installed**: Check that agent files exist:
   ```bash
   ls ${CODEX_HOME:-$HOME/.codex}/agents/blog-*.md
   ```
   Expected: `blog-researcher.md`, `blog-writer.md`, `blog-seo.md`,
   `blog-reviewer.md`

2. **Missing `Task` in tool permissions**: The sub-skill's YAML frontmatter must
   include `Task` in its `tool permissions` list. Check the sub-skill file:
   ```bash
   head -20 ${CODEX_HOME:-$HOME/.codex}/skills/blog-write/SKILL.md
   ```
   The `tool permissions` section should include `Task`.

3. **Codex version**: Agent spawning via `Task` requires a recent
   version of Codex. Update to the latest version.
...
1. Verify reference files exist:
   ```bash
   ls ${CODEX_HOME:-$HOME/.codex}/skills/blog/references/
   ```
2. Re-install references:
   ```bash
   cp skills/blog/references/*.md ${CODEX_HOME:-$HOME/.codex}/skills/blog/references/
   ```
...
**Causes**:
- **Research phase**: web search calls for statistics and images can take
...
1. **Check the version**: Ensure you have the latest `codex-blog` installed:
   ```bash
   cd codex-blog && git pull && ./install.sh
   ```

2. **Verify file integrity**: Compare installed files with the repository:
   ```bash
   diff ${CODEX_HOME:-$HOME/.codex}/skills/blog/SKILL.md skills/blog/SKILL.md
   ```
...
4. **Open an issue**: https://github.com/AgriciDaniel/codex-blog/issues
