...
def test_preflight_gate_5_flags_non_http_scheme_as_violation(tmp_path: Path) -> None:
    ...
    (tmp_path / "review.md").write_text("BLOCKING: NO (test)", encoding="utf-8")
    result = subprocess.run(
        [sys.executable, str(PREFLIGHT_PATH), "--draft", str(tmp_path),
         "--gate", "5", "--no-strict"],
        capture_output=True, text=True, check=False,
...
def test_reference_count_coherence() -> None:
    ...
    m = re.search(r"(\d+) references", agents_text)
    assert m, "AGENTS.md must declare reference doc count"
    declared_agents = int(m.group(1))
    assert declared_agents == actual, (
        f"AGENTS.md says {declared_agents}; filesystem has {actual}"
    )
