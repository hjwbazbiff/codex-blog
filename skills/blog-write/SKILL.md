...
4. Local pack (out of scope for blog content; use codex-seo for local)
...
Spawn a `blog-researcher` agent (or do inline research with web search):
...
7. **Find relevant YouTube videos** (2-3 per post):
   - Use `blog-google` youtube command or web search `site:youtube.com [topic] [year]`
...
## Delivery Contract Reminder

Always preserve `skills/blog/references/blog-delivery-contract.md`. Run `scripts/blog_preflight.py` for the publication gates and dispatch `blog-reviewer` for nonce-bound review before final approval.

## Additional Operational Detail

Load `skills/blog-write/references/operational-detail.md` when you need the full example library, edge-case handling, or extended execution notes.
