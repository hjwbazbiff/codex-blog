...
FLOW is an evidence-led 2026-search operating model published at `github.com/AgriciDaniel/flow` (CC BY 4.0 prompt content, MIT code), authored by Daniel Agrici. It treats modern discovery as a multi-surface system rather than a single SERP, and it enforces a strict provenance discipline on every public claim. codex-blog v1.7.0 integrates FLOW as the `blog-flow` sub-skill (which exposes the upstream stages and prompts) plus this alignment doc, which informs how every other blog skill in the suite plans, writes, optimizes, and audits content. The intent is that FLOW provides the principles and prompts; codex-blog provides the production tooling that applies them at scale.

## 2. The FLOW evidence triple (the rule blog-write enforces)

Every public statistic in a codex-blog post must satisfy three components.
...
| 4. Local pack | Map listings, Google Business Profile | Out of scope for blog content. Use `codex-seo` (`/seo local`, `/seo maps`) for local SEO. |
...
codex-blog operationalizes surfaces 1, 2, 3, and 5 directly. Surface 4 is delegated to codex-seo. When a blog post targets a query that also surfaces in a community (Reddit thread, YouTube comment), the writer should consider dual-surface thinking: optimize the post for extraction AND consider whether the same answer should also live in the community where the query is asked.
...
The Leverage stage (off-site authority, 1 prompt) and Local stage (11 GBP/citation prompts) are intentionally not exposed as top-level codex-blog commands. Leverage is reachable through `/blog flow prompts`. Local is delegated to codex-seo.
...
- Google API integration (`blog-google`: PSI, CrUX, GSC, GA4, NLP, YouTube, Keywords). FLOW references measurement; codex-blog provides API tooling.
...
Last updated 2026-04-27 with codex-blog v1.7.0 launch.
