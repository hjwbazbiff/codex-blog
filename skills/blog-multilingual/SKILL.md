...
license: MIT
compatibility: Requires codex-blog (blog-write). Optional integration with codex-seo (seo-hreflang) for richer hreflang validation.
metadata:
  author: AgriciDaniel
...
> Adapted from `codex-blog-multilingual` by Chris Mueller (AI Marketing Hub
...
from codex-seo is installed, delegate validation to it. Otherwise use the
...
- If codex-seo is installed, run `/seo hreflang multilingual/` for
  deeper validation.
```
