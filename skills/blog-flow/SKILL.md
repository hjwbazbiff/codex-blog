...
license: MIT
compatibility: Requires Codex and Python 3.11+ for the sync script
metadata:
  author: AgriciDaniel
...
FLOW is an evidence-led operating model built for the AI-search era. Codex Blog
...
3. State that local-SEO prompts are excluded by design; point users to
   `codex-seo` (`/seo flow local`) if they need them.
