...
Query Google NotebookLM notebooks directly from Codex for citation-backed
...
When invoked as a Codex subagent from blog-write or blog-researcher:
...
with no error. The calling workflow continues with web search-based research.
...
| MCP unavailable (internal) | Return silently: writing workflow uses web search |
...
- Local Codex only (not available in web UI)
