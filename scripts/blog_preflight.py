...
exists and that its BLOCKING decision is `NO` or legacy `false`.
...
HEAD_TIMEOUT = 10
USER_AGENT = "codex-blog/1.9.0 preflight (+https://github.com/hjwbazbiff/codex-blog)"
...
def gate_1_capability_discovery(draft_dir: Path) -> dict:
    ...
    agents = {a: (root / "agents" / f"{a}.toml").is_file() for a in REQUIRED_AGENTS + OPTIONAL_AGENTS}
...
def gate_4_content_review(draft_dir: Path) -> dict:
    ...
    with `BLOCKING: NO` AND a matching Nonce (v1.9.1).
    ...
    m = re.search(r"^BLOCKING:\s*(yes|no|true|false)\s*(?:\((.*?)\))?\s*$", text, re.IGNORECASE | re.MULTILINE)
    if not m:
        return _gate_result(
            4, "Content Review", False,
            ["review.md present but no `BLOCKING: YES|NO` line found at end of scorecard"],
            nonce_warnings,
        )
    blocking = m.group(1).lower() in {"yes", "true"}
